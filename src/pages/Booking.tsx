...
export default function Booking() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      // Perform the insert into the 'booking' table
        ...
        .insert([
          {
            ...
            check_in: '2024-05-12', // Formatted as YYYY-MM-DD
            check_out: '2024-05-15', // Formatted as YYYY-MM-DD
            guests: 1 // Integer value
          }
        ])
        .select();
      ...
      // Use the returned ID as the reservation reference
      ...
      alert('Failed to submit booking. Ensure your Supabase tables and policies are set up correctly.');
    } finally {
          ...
          <div className="lg:col-span-8">
            ...
            {step === 2 && (
              <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }}>
                ...
                <div className="mt-20">
                   <h4 className="text-label mb-10 text-primary">Elevate your stay</h4>
                   <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                      {[
                        { name: "Airport Pickup", price: "+฿600" },
                        { name: "Artisan Breakfast", price: "+฿150" },
                        { name: "Rooftop Pass", price: "+฿200" }
                      ].map(opt => (
                        <div key={opt.name} className="border border-white/5 p-8 bg-white/[0.01] hover:bg-primary/[0.03] hover:border-primary/50 transition-all cursor-pointer group">
                           <p className="text-[10px] font-black uppercase tracking-[0.2em] mb-4">{opt.name}</p>
                           <p className="text-2xl font-light italic opacity-40 group-hover:opacity-100 group-hover:text-primary transition-all">{opt.price}</p>
                        </div>
                      ))}
                   </div>
                </div>
              ...
              <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
                <div className="max-w-xl py-20">
                   ...
                   <p className="text-xl text-white/40 italic mb-10 max-w-sm leading-relaxed">
                     Your reservation at Soi Social BKK is confirmed. Check your email for full access details.
                   </p>
                   <div className="p-8 border border-white/10 bg-white/[0.02]">
                      <p className="text-[10px] font-black uppercase tracking-[0.3em] opacity-30 mb-4">Reservation ID</p>
                      <p className="text-2xl font-mono text-primary font-bold italic">#{bookingRef.toString().slice(0, 8).toUpperCase()}</p>
                   </div>
                </div>
              </motion.div>
            )}
             ...
             <div className="sticky top-32">
                <div className="border border-white/10 p-10 glass-card relative overflow-hidden">
                   <div className="absolute top-0 right-0 p-4 opacity-5 pointer-events-none">
                      <span className="text-6xl font-black italic">SOI.</span>
                   </div>
                   ...
                   {selectedRoomData ? (
                     <div className="space-y-10">
                        ...
                        <div className="space-y-4">
                           <div className="flex justify-between text-xs font-bold uppercase tracking-[0.2em] opacity-40 italic">
                             <span>May 12 (3 nights)</span>
                             <span>฿{selectedRoomData.price * 3}</span>
                           </div>
                           <div className="flex justify-between text-xs font-bold uppercase tracking-[0.2em] opacity-40 italic">
                             <span>Taxes & Service (10%)</span>
                             <span>฿{(selectedRoomData.price * 3 * 0.1).toFixed(0)}</span>
                           </div>
                        </div>
                ...
                <div className="mt-8 flex justify-center">
                   <p className="text-[8px] uppercase tracking-[0.5em] font-black opacity-20 hover:opacity-50 transition-opacity cursor-default">• NO HIDDEN FEES • SECURE PAY • REALTIME •</p>
                </div>
             </div>
          </div>
